...
# Inactivity dictionary is defined as account name as key, and inactivity as value.
# In this dictionary, we don't care about who the player is linked to.
inactivity_dictionary = {}

for claimer in claims_dictionary:
    for claim in claims_dictionary[claimer]: 
        if claim.name not in inactivity_dictionary: inactivity_dictionary[claim.name] = 0
        inactivity_dictionary[claim.name] += claim.inactivity

# Sort the inactivity dictionary by inactivity descending.
inactivity_dictionary = {account: inactivity_dictionary[account] for account in sorted(inactivity_dictionary, key=lambda account: inactivity_dictionary[account], reverse=True)}
...
with open("inactivity_output.txt", "w", encoding="utf-8") as file:
